...
function credPath(email) {
    const safe = email.replace(/[^a-zA-Z0-9_.@-]/g, "_");
    return path.join(TOKEN_DIR, `${safe}.json`);
}
async function saveCredential(email, refreshToken, scopes, clientId) {
...
async function loadCredential(email) {
    try {
        const data = JSON.parse(await fsPromises.readFile(credPath(email), "utf8"));
...
async function openBrowser(url, profileDir) {
...
async function runAuth(account, profileDir) {
    ...
    if (typeof address !== "object" || !address)
        fatal("Failed to bind local callback port.");
    ...
    if (!refresh)
        fatal("No refresh_token returned; check OAuth client settings and consent prompt.");
...
async function getAuthForEmail(email) {
    ...
    if (!cred)
        fatal(`No stored credentials for ${email}. Run auth first.`);
    const missing = OAUTH_SCOPES.filter((s) => !(cred.scopes ?? []).includes(s));
    if (missing.length) {
        fatal(`Stored token for ${email} is missing required scopes (${missing.join(", ")}). Please re-run auth for this account.`);
    }
    const clientId = ensureEnv("TEST_TOOL_GOOGLE_OAUTH_CLIENT_ID");
    const clientSecret = ensureEnv("TEST_TOOL_GOOGLE_OAUTH_CLIENT_SECRET");
...
async function getDriveForEmail(email) {
...
function loadSpec(specPath) {
    ...
    if (!spec.accounts?.length)
        fatal("Spec missing accounts");
    if (!spec.root || !spec.root.name)
        fatal("Spec missing root.name");
    return spec;
}
function ownerEmailForLabel(label, defaultLabel, map) {
    const resolved = label ?? defaultLabel;
    const email = map.get(resolved);
    if (!email)
        fatal(`Unknown owner label: ${resolved}`);
    return email;
}
async function createFolder(drive, name, parentId, verbose) {
    ...
    if (!id)
        throw new Error("Folder creation missing id");
    if (verbose)
        log(`Created folder ${name} -> ${id}`);
    return id;
}
async function createGoogleFile(drive, mimeType, name, parentId, verbose) {
    const res = await withRetry(() => drive.files.create({
    ...
    const id = res.data.id;
    if (!id)
        throw new Error("File creation missing id");
    if (verbose)
        log(`Created file ${name} -> ${id}`);
    return id;
}
async function uploadAsset(drive, mimeType, name, parentId, assetPath, verbose) {
    const res = await withRetry(() => drive.files.create({
        requestBody: { name, mimeType, parents: [parentId] },
    ...
    const id = res.data.id;
    if (!id)
        throw new Error("Upload missing id");
    if (verbose)
        log(`Uploaded ${name} (${assetPath}) -> ${id}`);
    return id;
}
async function copyFile(drive, sourceId, name, parentId, verbose) {
    const res = await withRetry(() => drive.files.copy({
        fileId: sourceId,
    ...
    const id = res.data.id;
    if (!id)
        throw new Error("Copy missing id");
    if (verbose)
        log(`Copied ${sourceId} -> ${id}`);
    return id;
}
async function linkFormToSheet(email, formId, sheetId, verbose) {
    ...
    if (!token)
        fatal(`Failed to get access token for ${email}`);
    ...
    const resp = await withRetry(() => fetch(url.toString(), {
        method: "GET",
        headers: {
    ...
    const linkedSheetId = json?.linkedSheetId ?? undefined;
    if (linkedSheetId === sheetId) {
        if (verbose)
            log(`Form ${formId} already linked to sheet ${sheetId}`);
        return;
    }
    if (!linkedSheetId) {
        log([
...
async function buildTree(node, parentId, ownerLabel, labelToEmail, defaultLabel, verbose) {
    ...
    if (mimeType === "copy") {
        if (!node.sourceId)
            fatal(`copy node ${node.name} missing sourceId`);
    ...
    if (node.asset) {
        ...
        if (!fs.existsSync(assetPath))
            fatal(`Asset not found: ${assetPath}`);
...
async function listDirectChildren(drive, parentId) {
    ...
    do {
        ...
        (res.data.files ?? []).forEach((f) => {
            if (f.id)
                children.push({ id: f.id, mimeType: f.mimeType ?? undefined, name: f.name ?? undefined });
        });
        pageToken = res.data.nextPageToken ?? undefined;
    } while (pageToken);
    return children;
}
async function purgeFolderContents(drive, parentId, verbose) {
    ...
    for (const child of children) {
        ...
        if (verbose)
            log(`Deleted ${child.name || child.id}`);
    }
}
async function ensureAccessForAll(rootId, accounts) {
    for (const account of accounts) {
        const drive = await getDriveForEmail(account.email);
        try {
            await withRetry(() => drive.files.get({
                fileId: rootId,
                supportsAllDrives: true,
                fields: "id,name",
...
async function handlePopulate(specPath, rootId, verbose) {
...
async function handleClean(rootId, account) {
    const creds = await listCredentials();
    if (creds.length === 0)
        fatal("No stored credentials. Run auth first.");
...
async function main() {
    ...
    switch (command) {
        ...
        case "populate": {
            ...
            if (!values.spec || !values["root-id"])
                fatal("populate requires --spec and --root-id");
