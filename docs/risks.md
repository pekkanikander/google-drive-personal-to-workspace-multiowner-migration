...
| --- | --- | --- | --- | --- |
| R1 | **Service account enumeration fails** — the Workspace service account cannot see or classify all items in a shared personal Drive tree. | Blocks manifest generation and admin planning. | Open | Execute Architecture Spike 3 to verify `files.list` coverage, owners, shortcuts, and permissions. Adjust design if gaps emerge. |
